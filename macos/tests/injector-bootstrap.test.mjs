...
const registrationStart = source.indexOf("record.earlyScriptId = await registerEarlyForRecord");
...
assert.match(source, /if \(!record\.needsLoadFallback\) return;/,
  "Load-event reinjection must stay inert until early injection falls back.");
assert.match(source, /Page\.removeScriptToEvaluateOnNewDocument/,
  "Watcher shutdown and theme refresh must unregister persistent Page scripts.");

console.log("PASS: macOS early injection is shell-guarded, generation-safe, ordered before probing, and fallback-scoped.");
