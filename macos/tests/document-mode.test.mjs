...
const renderer = await fs.readFile(path.join(macosRoot, "assets", "renderer-inject.js"), "utf8");
...
assert.match(renderer, /const ensureStreamTracking = \(\) =>/);
assert.match(renderer, /ensureStreamTracking\(\);\s*ensureProseWrapper\(\);/);
assert.match(renderer, /发送\(\?:消息\)\?/);
assert.match(renderer, /send\(\?: message\)\?/);
