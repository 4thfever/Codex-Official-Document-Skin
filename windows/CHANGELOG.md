...
- 新增只读 `doctor-dream-skin.ps1` 预检：在安装前报告官方 Store 包、Node.js、`config.toml`、端口、日常快捷方式执行策略和 Dream Skin state；不会安装或配置 Codex、关闭进程、创建文件或修改策略。安装器会在任何写入前复用关键预检，失败时不产生安装副作用。
- Windows 内置主题收敛为唯一的 `CODEX Document` 预设；移除不再支持的历史预设与相关产品文案，用户导入、保存和切换自定义主题的能力保持不变。
